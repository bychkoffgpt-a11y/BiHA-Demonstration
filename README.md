...
- фактическая роль (master/slave) определяется запросом `pg_is_in_recovery()`; экран производительности также ориентируется на эту фактическую роль, а не только на `role_hint`, поэтому после failover графики продолжают читать метрики с текущего primary;
...
- `disk_device` — имя блочного устройства для мониторинга (`/dev/sda`, `sdb`, `nvme0n1` и т.п.); если указано, приложение сначала ищет точное совпадение, затем подходящие разделы (`sda1`, `nvme0n1p1`), а если устройство не найдено — аккуратно откатывается к агрегированию по доступным дискам, чтобы графики не залипали в нуле.
...
- **чтение** — точечный поиск по `aid`, агрегат по `bid`, join `accounts+tellers+branches`, top-20 по балансу в филиале;
...
Такое разделение уменьшает конкуренцию за блокировки в `rw-master`: приложение больше не держит range-update по accounts и не обновляет общие summary-строки в каждой транзакции.
...
- pgbench-версии облегчённых профилей чтения, базовой write-части и дополнительной write-части;
...
По умолчанию экран производительности сам подставляет рабочую БД из активной нагрузки (или из DSN текущего primary), поэтому `TPS` и `WAL` не требуют ручного ввода `postgres`, если нагрузка направлена в другую БД.
...
3. Запустите нагрузку кнопкой `▶️ Запустить нагрузку` и при необходимости меняйте `Количество клиентов`/`Потоков на клиента` прямо во время работы — генератор подстроится автоматически.
