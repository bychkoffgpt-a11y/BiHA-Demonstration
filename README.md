...
- `Read ratio` — доля чтений (0.0..1.0) только для смешанных профилей `rw-master` и `rw-master-r-slave`.
...
- `Reset counters` — сброс счётчиков генератора и `pg_stat_*` статистик на всех узлах;
- `Reset all caches` — очистка page cache / dentries / inodes на всех узлах кластера по SSH (`sync; echo 3 > /proc/sys/vm/drop_caches`).
