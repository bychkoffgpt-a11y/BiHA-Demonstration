...
- **чтение+запись** — короткая транзакция с диапазонным чтением, диапазонным обновлением `pgbench_accounts`, обновлением `pgbench_tellers`/`pgbench_branches`, вставкой в `pgbench_history` и контрольным агрегатом по истории.
...
- pgbench-версии облегчённых профилей чтения и чтения+записи;
