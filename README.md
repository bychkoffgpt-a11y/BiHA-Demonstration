...
Это **демонстрационное приложение на Streamlit** для мониторинга и тестирования отказоустойчивости PostgreSQL-кластера.
Приложение:
- подключается к нескольким узлам PostgreSQL по DSN;
- показывает текущую роль узла (master/slave) и базовые метрики;
- генерирует нагрузку в стиле pgbench по выбранному профилю, числу клиентов и числу потоков на клиента;
...
- **чтение** — сценарий из двух частей:
  ...
  - дополнительная часть выполняется в 1 из 4 транзакций чтения и включает расширенный точечный запрос по `aid`, агрегат по `bid` без `count(*)` (сумма, среднее и максимум баланса), соединение `accounts+tellers+branches` и выборку 20 самых больших балансов в филиале;
- **чтение+запись** — короткая транзакция из двух частей:
  - базовая часть в каждой транзакции записи: чтение и обновление одной строки `pgbench_accounts`;
  - дополнительная часть примерно в каждой четвёртой транзакции записи: обновление одной строки `pgbench_tellers`, вставка в `pgbench_history` и контрольный агрегат по истории без `count(*)` — сумма изменений и время последнего события.
...
- Параметр размера БД вынесен на страницу инициализации и не отображается на главной странице `Демонстрация кластера PostgreSQL BiHA`.
...
На странице `SQL команды инициализации и нагрузки` отображаются все SQL-команды:
- создания, очистки и наполнения таблиц;
- сценарии pgbench для базовой и дополнительной части профиля чтения, а также для базовой и дополнительной части записи;
- SQL-эквиваленты этих профилей, которые приложение выполняет через `psycopg`;
- пояснение, что в нагрузочных сценариях больше не используется `count(*)`;
...
- **шаг агрегации** (`2 / 5 / 10 / 15 / 30 сек`);
